...
if (!supabaseUrl || !supabaseKey) {
  console.error("❌ FEIL: SUPABASE_URL eller SUPABASE_KEY er tom!");
  console.error("SUPABASE_URL:", supabaseUrl ? "OK" : "MANGLER");
  console.error("SUPABASE_KEY:", supabaseKey ? "OK" : "MANGLER");
  process.exit(1);
}
...
console.log("✓ Supabase-verdier funnet");
const jwtRole = getJwtRole(supabaseKey);
if (jwtRole) {
  console.log(`✓ Supabase key role: ${jwtRole}`);
  if (jwtRole !== "service_role") {
    console.error("❌ Denne nøkkelen er ikke service_role. Upsert vil bli blokkert av RLS.");
    process.exit(1);
  }
} else {
  console.log("⚠️ Klarte ikke å lese role fra Supabase key, men fortsetter.");
}
